#!/usr/bin/env python3
"""
Docker Status Monitor with Notification System
Features:
- Real-time container status monitoring
- Bell icon with notification count badge  
- Collapsible notification sidebar
- Container update detection with version comparison
- One-click container updates (pulls latest, preserves data)
...
class DockerStatusMonitorWithNotifications:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.tk.call('tk', 'appname', 'docker-status-monitor')
        self.root.title("🐳 Docker Status Monitor")
        self.root.geometry("750x450")  # Default without sidebar
...
'''
    ...
    def _show_update_progress(self, project_name, status):
        ...
        self.root.title(f"Docker Status Monitor  - {project_name}: {status}")
